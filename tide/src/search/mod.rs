...
impl Sink for LineCollector {
    ...
    fn matched(&mut self, _searcher: &Searcher, mat: &SinkMatch<'_>) -> Result<bool, io::Error> {
        ...
        self.lines.push(SearchItem::new(
            &content,
            (&self.path, mat.line_number()),
            SearchItemType::TEXT,
        ));
...
pub fn search(cwd: &Path, query: &str) -> SearchHandle {
        ...
        walker.run(move || {
            ...
            Box::new(move |entry| {
                ...
                if entry.file_type().is_some_and(|ft| ft.is_dir()) {
                    let _ = thread_local_tx.send(SearchItem::new(
                        &path_str,
                        (path, None),
    ...
    let t2 = thread::spawn(move || {
        ...
        build_rx.into_iter().par_bridge().for_each_with(
